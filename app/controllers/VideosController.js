...
const createVideo = async(req, res, next) => {
    try {
        ...
        const data = await Chapter.updateOne(
            { _id: chapterId },
...
const updateVideo = async(req, res, next) => {
    try {
        ...
        const data = await Chapter.updateOne(
            { _id: chapterId, 'videos._id': videoId },
            { $set: { 'videos.$.title': newVideo.title, 'videos.$.videoUrl': newVideo.videoUrl, 'videos.$.duration': newVideo.duration }}
        );
        if(data.modifiedCount === 0) {
            return res.status(404).send(resSuccess("Chapter not found", null))
        }
